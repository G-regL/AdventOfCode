...
func main() {
	...
	inspectedItems := make([]int, len(inputMonkeys))

	for round := 1; round <= 20; round++ {
		fmt.Printf("----- Round %2d -----\n", round)

		for mID, Monkey := range Monkeys {
			inspectedItems[mID] += len(Monkey.items)
			//fmt.Printf("Monkey %d:\n", mID)
	...
	fmt.Println("\n-----------------------")
	for mID, count := range inspectedItems {
	...
	fmt.Println("\n-----------------------")
	fmt.Println("Part 1 Monkey Business:", inspectedItems[len(inspectedItems)-1]*inspectedItems[len(inspectedItems)-2])
	fmt.Printf("Took %s\n", time.Since(start))
}
